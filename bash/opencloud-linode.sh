...
#linnode服务器位置
region_linode(){
    json=`cat <(curl -Ls https://raw.githubusercontent.com/jasoduncan/open_cloud/main/linode/region)`
...
#linode服务器镜像
image_linode(){
    json=`cat <(curl -Ls https://raw.githubusercontent.com/jasoduncan/open_cloud/main/linode/image)`
    o=`echo $json| jq ".opencloud | length"`
...
#linode服务器大小
size_linode(){
    json=`cat <(curl -Ls https://raw.githubusercontent.com/jasoduncan/open_cloud/main/linode/size)`
    o=`echo $json| jq ".opencloud | length"`
